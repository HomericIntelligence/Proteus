...
specifies the multi-agent coordination protocols for Proteus within the
HomericIntelligence distributed agent mesh, plus project overview,
architecture, and development guidance for agents working in this repo.
...
Proteus centralizes all pipeline logic for the HomericIntelligence
ecosystem using Dagger (TypeScript SDK), manages OCI image builds, runs
test suites, promotes images via Skopeo, and orchestrates cross-repo
GitHub Actions dispatch events.
...
The payload contracts for each hop are specified in the handoff sections
